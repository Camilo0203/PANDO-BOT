...
function createQueueCustomId(action, ownerId, sessionId, page) {
  ...
  if (action === QUEUE_ACTIONS.CLOSE) {
    return assertCustomIdLength(`${QUEUE_CUSTOM_ID_PREFIX}:${action}:${safeOwnerId}:${safeSessionId}`);
  }
  ...
  return assertCustomIdLength(`${QUEUE_CUSTOM_ID_PREFIX}:${action}:${safeOwnerId}:${safeSessionId}:${safePage}`);
}
