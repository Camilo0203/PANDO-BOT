...
function createNowPlayingEmbed(track, player, tier, language = "en") {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: t(language, "author"), value: truncate(track?.author || t(language, "unknown"), 100), inline: true },
      { name: t(language, "source"), value: formatSource(track?.sourceName, language), inline: true },
      { name: t(language, "duration"), value: formatDuration(track?.length), inline: true },
      { name: t(language, "volume"), value: `${Number.isFinite(Number(player?.volume)) ? Number(player.volume) : 100}%`, inline: true },
      { name: t(language, "loop"), value: formatLoop(player?.loop, language), inline: true },
      { name: t(language, "remaining_queue"), value: t(language, "track_count", { count: queueSize }), inline: true }
    )
    .setFooter({
      text: footerText([
...
function addedToQueueEmbed(track, position, tier, language = "en") {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: t(language, "author"), value: truncate(track?.author || t(language, "unknown"), 100), inline: true },
      { name: t(language, "duration"), value: formatDuration(track?.length), inline: true },
      { name: t(language, "queue_position"), value: `#${Math.max(1, Number(position) || 1)}`, inline: true }
    )
...
function playlistAddedEmbed(playlistName, count, tier, language = "en") {
  return createMusicSuccessEmbed(
    ...
    t(language, "playlist_description", { playlistName: truncate(playlistName, 120), count }),
    { tier, language }
  );
}
...
function createSearchResultEmbed(tracks, query, options = {}) {
  ...
  const title = language === "es" ? `🔍 Resultados de búsqueda: "${query}"` : `🔍 Search Results: "${query}"`;
