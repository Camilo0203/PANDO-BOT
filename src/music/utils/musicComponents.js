...
function createQueuePaginationControls({ ownerId, sessionId, page = 1, totalItems = 0, language = "en" }) {
  const pagination = getQueuePagination(totalItems, page);
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(createQueueCustomId(QUEUE_ACTIONS.PREVIOUS, ownerId, sessionId, pagination.previousPage))
      .setLabel(t(language, "queue_previous"))
      .setEmoji("\u25C0\uFE0F")
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(pagination.previousDisabled),
    new ButtonBuilder()
      .setCustomId(createQueueCustomId(QUEUE_ACTIONS.NEXT, ownerId, sessionId, pagination.nextPage))
      .setLabel(t(language, "queue_next"))
      .setEmoji("\u25B6\uFE0F")
      .setStyle(ButtonStyle.Primary)
      .setDisabled(pagination.nextDisabled),
    new ButtonBuilder()
      .setCustomId(createQueueCustomId(QUEUE_ACTIONS.CLOSE, ownerId, sessionId))
...
function createSearchSelectMenu(tracks, userId, options = {}) {
  ...
  const placeholder = language === "es" ? "Selecciona una canción..." : "Select a song...";
